# Input loading and shared statistical helpers for age and tumor-timing analyses.
...
ptc_age_load <- function(input_dir, file, object) {
  ...
  ptc_external_directory(dirname(path),'Input file directory')
...
ptc_age_result <- function(id, expression) {
  tryCatch({
    ...
    c(list(panel_id = id, status = 'generated'), value)
...
ptc_age_exposure <- function(data, clinical, replace_clinical_na = FALSE) {
  ...
  # Latency models first replace NA in matched clinical rows by zero;
