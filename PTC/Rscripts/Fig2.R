# Figure 2: clonal and subclonal chromosome 22q loss.
# Panel a computes separate copy-number states; panel d displays raw P values.
ptc_chr22_inputs <- function(input_dir) {
  ...
  # Classify each tumor by its longest chr22 segment; missing values become neutral.
...
run_Fig2 <- function(input_dir,output_dir,panels=letters[1:4]) {
  ...
  if ('a' %in% panels) results[['Fig. 2a']] <- ptc_capture('Fig. 2a', {
    ...
    clone <- ptc_cnv_frequency(x$cnv,x$ngspurity,ptc_reference('hg38_cytoBand.txt.gz'),'clone')
    subclone <- ptc_cnv_frequency(x$cnv,x$ngspurity,ptc_reference('hg38_cytoBand.txt.gz'),'subclone')
    ...
    list(paths=ptc_save(p,output_dir,'Fig2a_states',14,8),
         scope='Separate clonal and subclonal copy-number states. Agreement with the intended state selection requires author confirmation.')
  })
  if ('b' %in% panels) results[['Fig. 2b']] <- ptc_capture('Fig. 2b', {
    ...
    list(paths=ptc_save(p,output_dir,'Fig2b',8,10),clonal_loss_n=length(x$clonal_loss),subclonal_loss_n=length(x$subclonal_loss),
         scope='Longest-segment chr22 classification; neutral samples are displayed as white rows.')
  })
  if ('c' %in% panels) results[['Fig. 2c']] <- ptc_capture('Fig. 2c', {
    ...
    list(paths=ptc_save(p,output_dir,'Fig2c',9,6),n_segment_observations=nrow(dat),n_unique_tumors=n_distinct(dat$Tumor_Barcode),
         scope='Each observation is a qualifying segment; multiple segments can belong to one tumor.')
  })
  if ('d' %in% panels) results[['Fig. 2d']] <- ptc_capture('Fig. 2d', {
    ...
    list(paths=ptc_save(p,output_dir,'Fig2d_raw_p',12,7),n_tests=nrow(dat),adjustment='Holm adjustment for event labels',
         scope='Raw-P display with Holm-adjusted event labels. The adjusted-P figure display is not implemented.')
  })
  results
}
