# Minimal shared transformations from TP_Comparision.R (source ranges in provenance).
...
ptc_event_matrix <- function(wgsdata, data_top0) {
  # Retain the historical >10 event-row threshold and pooled fusion category.
...
ptc_volcano <- function(data, x, facet=FALSE, bonferroni=FALSE) {
  ...
  # The original upper line is 0.05/12, not an FDR threshold. Keep its value.
