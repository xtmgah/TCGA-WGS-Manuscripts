# Fig.5: age, exposure and driver-event associations.
# Source evidence: TP_Comparision.R 2920-3001, 3046-3064, 3180-3215,
# 3363-3370 and 3453-3553. Read provenance/ for exact historical mappings.
# Run from TCGA-PTC after sourcing functions/runtime.R and age_timing_helpers.R.
run_Fig5 <- function(input_dir, output_dir) {
  ...
  results[['Fig. 5a']] <- ptc_age_result('Fig. 5a', {
    # Preserve the historical denominator: only tumors with at least one
    # curated alteration enter the wide matrix (the right_join was commented).
    ...
    list(files = ptc_save(p, output_dir, 'Fig5a', 10, 6), statistics = fits,
      notes = 'Historical alteration-positive-only matrix denominator retained. Dose capped at 1000; missing age at exposure=100. BH correction within predictor.')
  })
  ...
  for (id in names(frequency_specs)) {
    ...
    results[[id]] <- ptc_age_result(id, {
      ...
      list(files = ptc_save(p, output_dir, spec[4], 10, 6), statistics = counts,
        notes = 'Original age categories retained; 70 years is included in 60–70. Zero numerator groups set to zero.')
    })
  }

  results[['Fig. 5e']] <- ptc_age_result('Fig. 5e', {
    # Explicit keys duplicate the historical natural join on barcode AND diagnosis
    # age without accidental suffixing of age_at_diagnosis.
    ...
    list(files = ptc_save(p, output_dir, 'Fig5e', 10, 6), statistics = stats,
      notes = 'Uses the 1x acceleration MRCA intermediate. Diagnosis age is not included in this historical latency model.')
  })
  ...
  results[['Fig. 5f']] <- ptc_age_result('Fig. 5f', {
    ...
    list(files=ptc_save(p,output_dir,'Fig5f',10,6),statistics=stats,
      notes='Historical p.adjust default is Holm. Scientific computation preserved; cleaned axis explicitly says Holm-adjusted P. Original axis said FDR. Gene inclusion count>10 before deduplication.')
  })
  results[['Fig. 5g']] <- list(panel_id='Fig. 5g', reproducibility_status='source code partially identified',
    notes='TP_MRCA_drivergenes_association.pdf recovered from Keynote. Nearby latency association code supplies plausible shared preparation, but the exact MRCA-response model/export is absent. No replacement model was fabricated.')
  results
}
