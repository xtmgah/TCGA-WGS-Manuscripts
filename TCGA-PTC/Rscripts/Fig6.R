# Fig6a/b occupy the two halves of one imported source PDF.
# Source: Segmented_breakpoint_analysis.R 10-113; input TP_Comparision.R 1697-1747.
# Existing scientific discrepancies are preserved and recorded in run metadata.
run_Fig6 <- function(input_dir, output_dir) {
  ...
  result <- ptc_age_result('Fig. 6a; Fig. 6b', {
    ...
    # Clone==Y and probability-weighted SBS1+SBS5 sum are preserved.
    ...
    # Base linear model (adjust for covariates you want in the mean curve)
    set.seed(1) # Added to stabilize bootstrap restarts; historical code did not specify a seed.
    fit_tcga <- lm(value ~ age_at_diagnosis + Tumor_Purity + Sex, data = tcga)

    # Segmented model; start around 45–55 as an initial guess
    ...
    # ---- 3) OPTIONAL: Chornobyl panel with simple linear fit ----
    ...
    list(files=ptc_save(combined,output_dir,'Fig6',14,6),statistics=statistics,
      notes='Original composite maps to both panels. Highlight is BRAF+TERT mutation-driver intersection. Chornobyl drawn line unadjusted, slope label adjusted. Segmented restart seed=1 added.')
  })
