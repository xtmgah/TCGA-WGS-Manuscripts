# Fig. 1: cohort characteristics, driver mutations and structural landscape.
# Run from TCGA-PTC after sourcing functions/ptc_runtime.R. Read provenance/
# for distinctions between exact export code and related historical code.
run_Fig1 <- function(input_dir, output_dir, panels=letters[1:9]) {
  ...
  if ('a' %in% panels) results[['Fig. 1a']] <- ptc_capture('Fig. 1a', {
    ...
    # These three exports have exact source code. Final study2_overview.pdf
    # assembly code was not recovered; these files are explicitly related.
    list(age=ptc_save(age_plot,output_dir,'Fig1a_related_age',8,6),
         depth=ptc_save(depth_plot,output_dir,'Fig1a_related_depth',10,6),
         purity=ptc_save(purity_plot,output_dir,'Fig1a_related_purity',8,6),
         scope='Related historical components only; final overview generator not recovered.')
  })
  ...
  if ('b' %in% panels) results[['Fig. 1b']] <- ptc_capture('Fig. 1b', {
    ...
    list(paths=ptc_save(p,output_dir,'Fig1b',10,6),n_cohort=nrow(wgsdata),n_driver_genes=nrow(dat),scope='Saved IntOGen object and exact frequency method.')
  })
  ...
  if ('c' %in% panels) results[['Fig. 1c']] <- ptc_capture('Fig. 1c', {
    ...
    fai <- readr::read_tsv('data/refs/hg38_primary.fai',col_names=FALSE,show_col_types=FALSE) %>%
    ...
    for (cohort_name in names(study_color2)) {
      ...
      outputs[[cohort_name]] <- ptc_save(p,output_dir,paste0('Fig1c_related_',gsub('[^A-Za-z0-9]','_',cohort_name)),12,4)
    }
    list(paths=outputs,scope='Exact per-cohort historical method; combined assembly and gene annotations are not recovered.')
  })
  ...
  for (panel in intersect(c('d','e','f'),panels)) {
    ...
    results[[id]] <- ptc_capture(id, {
      ...
      cytoband <- read.table(gzfile('data/refs/hg38_cytoBand.txt.gz'),sep='\t',header=FALSE,stringsAsFactors=FALSE)
      draw <- function() {
      ...
      tryCatch(draw(),finally=grDevices::dev.off())
      list(paths=paths,n_events=nrow(dat),scope='Exact paired cohort/export selection from commented historical branches; local hg38 cytobands.')
    })
  }
  ...
  for (panel in intersect(c('g','h','i'),panels)) {
    ...
    results[[id]] <- ptc_capture(id, {
      ...
      colors <- readr::read_csv('data/refs/oncoplot_colors.csv',show_col_types=FALSE)
      ...
      list(paths=ptc_save(p,output_dir,paste0('Fig1',panel),12,8),n_tumors=length(samples),scope='Exact saved curated events and original ordering; cohort selected explicitly; canvas enlarged for readable type.')
    })
  }
  results
}
